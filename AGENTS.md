...
ocpg is a **single-file** OpenCode plugin (`ocpg.ts`): a Postgres-backed persistent memory layer. It registers a `ctx.session.hook("context", ...)` hook (injects memories into system context), a `ctx.session.hook("prompt", ...)` hook (deterministic "remember that..." capture, no LLM), and seven tools via `ctx.tool.transform`: `memory_recall`, `memory_remember`, `memory_forget`, `memory_update`, `memory_consolidate`, `memory_tags`, `memory_retag`. The module exports **only `default`**; test internals are attached via `Object.assign(ocpg, { __internals })` (`import ocpg from "../ocpg"; const { __internals } = ocpg;`).
...
- **`memory_retag`** renames a tag across every row in one `UPDATE`, guarded by `visibleRows()` (same boundary as forget/update, not `memory_tags`' `global` flag - a write's reach is inherent to the WHERE clause, not an opt-in). `array_replace()` alone does **not** deduplicate: a row already tagged both `old` and `new` would end up with `new` twice, so the query wraps it in `ARRAY(SELECT DISTINCT unnest(...))` - verified directly against Postgres, not assumed. Clears the whole `injectionCache` (not just the caller's directory) on any actual rename, since a renamed `stack_fact`/`episodic` tag can appear in every project's cached block.
...
- `README.md`'s tool list/count can lag `ocpg.ts` - happened once already (`memory_tags`/`dryRun` shipped before README's "Five agent tools" section was updated to match). `ctx.tool.transform`'s `editor.add()` calls in `ocpg.ts` are the source of truth; update README (and `deploy/README.md` if the schema changed) in the same commit as a tool addition, not a follow-up.
