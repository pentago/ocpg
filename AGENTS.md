...
- Bun SQL array params need `sql.array(...)`: a raw JS array in a template produces `malformed array literal` under bun 1.4.2 (this silently broke `memory_remember` once — its test false-passed because `.catch(() => {})` swallowed the failed assertion inside `sql.begin`).
