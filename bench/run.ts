...
import { PARAPHRASES, PROJECTS, TOPICS, benchDbName, makeSql, pick, rng, thesaurusContent } from "./config.ts";
...
type BenchRow = { id: number; content: string; project: string };
type Case = { text: string; topicIdx: number; kind: "direct" | "paraphrase" };

const sanitizeOr = (q: string): string => (q.toLowerCase().match(/[a-z0-9]+/g) ?? []).slice(0, 24).join(" | ");
const sanitizeAnd = (q: string): string => (q.toLowerCase().match(/[a-z0-9]+/g) ?? []).slice(0, 24).join(" & ");
...
function buildCorpusWords(rows: BenchRow[]): Map<number, string[]> {
...
function buildMix(rand: () => number, corpusWords: Map<number, string[]>): Case[] {
  const cases: Case[] = [];
...
function score(returned: Array<{ content: string }>, relevant: Set<string>): { recall: number; mrr: number; prec: number } | null {
